...
from dataset_consep import MyDataset
from utils import *
...
RADIOLOGY_DATA_PATH = os.path.join(base_data_dir,'monuseg') # Containing two folders named data and test
...
def main():
    ...
    IMG_SIZE = 512
    wandb.init(project = 'TNBC', name = "Swin_UNet" + str(random_seed) )
    ...
    for epoch in range(num_epoch):
        ...
        for phase in ['train','val']:
            ...
            for i, d in enumerate(dataloaders[phase]):
                ...
                #output1,_ = model(img) 
                output1 = model(img) 
                #print(f'output img: {output1}') Logits
                # output1 = torch.stack([output1, -output1], dim=1)
                # print(f'output img shape: {output1.shape}')

                ce_l = ce_loss1(output1, target_one_hot)
                #ce_l = ce_loss1(output1, semantic_seg_mask)
                dice_l = dice_loss1(output1, semantic_seg_mask.float(), softmax=True)
                
                loss_seg = 0.4 * ce_l + 0.6 * dice_l
                
                if phase == 'train':
                    wandb.log({"CE Loss/train":ce_l, "Dice Loss/train":dice_l})
                elif phase == 'val':
                    wandb.log({"CE Loss/val":ce_l, "Dice Loss/val":dice_l})
            ...
            print(f"{phase} Epoch: {epoch+1}, loss: {epoch_loss_seg}, lr: {optimizer.param_groups[0]['lr']}, time: {e-s}")
           
            if phase == 'train':
                train_loss.append(epoch_loss_seg)
                wandb.log({"Seg Loss/train": epoch_loss_seg})
                wandb.log({"epoch time/train": e-s})
            else:
                test_loss.append(epoch_loss_seg)
                wandb.log({"Seg Loss/val": epoch_loss_seg})
...
########################## Testing ##############################
    ...
    "Test/Specificity": average_spec * 100.0,
    ...
    log_entry = f"{dataset}, 'p0', {random_seed}, {average_dice*100.0}, {average_iou*100.0}, {average_f1*100.0}, {average_accuracy*100.0}, {average_prec*100.0},{average_sens*100.0}, {average_spec*100.0}, {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}, {model_part}\n"
