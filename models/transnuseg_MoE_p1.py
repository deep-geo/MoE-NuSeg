...
class shiftedBlock(nn.Module):
    ...
    def flops(self): # added by ray
        flops = 5*256*768

#         # Input normalization (LayerNorm)
#         flops += 2 * x.size(1)  # Assuming 2 FLOPs per element for normalization

#         # Pass through the custom MLP (shiftmlp)
#         # Assuming shiftmlp has linear layers, element-wise activations (ReLU or similar), and LayerNorm
#         mlp_input_size = x.size(1)
#         mlp_hidden_size = self.mlp.hidden_features
#         flops += 2 * mlp_input_size * mlp_hidden_size  # FLOPs for linear layer
#         flops += mlp_hidden_size  # FLOPs for activation function
#         flops += 2 * mlp_hidden_size  # FLOPs for LayerNorm

#         # Skip connection (element-wise addition)
#         flops += x.size(1)  # Assuming 1 FLOP per element for addition
...
class SwinTransformerBlock_up(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        #print(f"MLP input/output dim={self.dim}, MLP hidden dim={expert_hidden_dim}")
...
class BasicLayer_up(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 drop_path=0., norm_layer=nn.LayerNorm, upsample=None, use_checkpoint=False, num_experts=3):
...
class TransNuSeg(nn.Module):
    ...
    def __init__(self, img_size=512, patch_size=4, in_chans=3, num_classes=2,
        ...
        if self.final_upsample == "expand_first":
            ...
            self.edge_transform = nn.Linear(embed_dim, embed_dim)
            self.edge_output = nn.Conv2d(in_channels=embed_dim, out_channels=self.num_classes, kernel_size=1, bias=False)
            
            self.cluster_transform = nn.Linear(embed_dim, embed_dim)
            self.cluster_output = nn.Conv2d(in_channels=embed_dim, out_channels=self.num_classes, kernel_size=1, bias=False)
